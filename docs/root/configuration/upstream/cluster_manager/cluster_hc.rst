...
在每一个健康检查周期内，所有的“发送”字节都会发送至目标服务器。

当检查响应时，"fuzzy" 匹配会被执行以确保找到每一个块，且按照指定的顺序，但不一定是连续的。因此，在上述的示例中，"04" 可以插在 "02" 和 "03" 的响应之间，且检查也将会通过。这么做是为了支持把不确定性数据，比如数据，插入响应中的协议。

较复杂的健康检查模式，诸如 send/receive/send/receive，现在还不可能。
