...
.. csv-table::
   ...
   dropped_headers_with_underscores, Counter, 名称中包含下划线的被丢弃的 header 总数. 这个统计可以通过设置 :ref:`headers_with_underscores_action config setting <envoy_v3_api_field_config.core.v3.HttpProtocolOptions.headers_with_underscores_action>`.
   metadata_not_supported_error, Counter, HTTP/1编码期间被丢弃的元数据总数
   response_flood, Counter, 由于响应泛洪而关闭的连接总数
   requests_rejected_with_underscores_in_headers, Counter, 由于 header 包含下划线而导致拒绝的请求总数。这个统计可以通过设置 :ref:`headers_with_underscores_action config setting <envoy_v3_api_field_config.core.v3.HttpProtocolOptions.headers_with_underscores_action>`.
...
.. csv-table::
   ...
   dropped_headers_with_underscores, Counter, 名称中包含下划线的被丢弃的 header 总数. 这个统计可以通过设置 :ref:`headers_with_underscores_action config setting <envoy_v3_api_field_config.core.v3.HttpProtocolOptions.headers_with_underscores_action>`.
   header_overflow, Counter,由于 header 大于参数 :ref:`configured value <envoy_v3_api_field_extensions.filters.network.http_connection_manager.v3.HttpConnectionManager.max_request_headers_kb>` 而重置的连接总数
   ...
   requests_rejected_with_underscores_in_headers, Counter, 由于 header 名称包含下划线而被拒绝的请求总数。这个统计可以通过设置 :ref:`headers_with_underscores_action config setting <envoy_v3_api_field_config.core.v3.HttpProtocolOptions.headers_with_underscores_action>`.
