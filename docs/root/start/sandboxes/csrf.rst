...
前端有一个字段用来输入你希望发送 POST 请求的远程域，以及一个用来选择远程域的 CSRF 强制执行的单选按钮。CSRF 强制追星选择有： 
...
如果你浏览 http://localhost:8001/stats，你将能够看到后端的所有 Envoy 统计。当你从后端集群发出请求时，你应该能够看到无效和有效来源的 CORS 统计信息在增加。
