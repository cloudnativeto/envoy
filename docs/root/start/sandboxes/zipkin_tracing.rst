...
Zipkin 追踪沙盒 是使用 `Zipkin <https://zipkin.io/>` 实现 Envoy 的 :ref:`请求追踪`功能。
...
端口 ``8000`` 会被通过 docker compose 暴露（参见 :repo:`/examples/zipkin-tracing/docker-compose.yaml`）。
...
但是，为了充分利用跟踪，应用端必须在调用其他服务的时候传递 Envoy 生成的 trace 的请求头信息。
在我们提供的沙箱例子中，作为 service1 的简单 flask 应用（参见 :repo:`/examples/front-proxy/service.py` 中的跟踪函数）在访问 service2 的时候传递了 trace 的请求头信息。
...
您现在可以通过 envoy 向 service1 发送请求，如下所示：
...
使用您的浏览器访问 http://localhost:9411。
你应该看到 Zipkin 仪表板。
将服务设置为 “front-proxy”，并将开始时间设置为在测试（步骤2）开始前几分钟并按回车。
你应该能看到来自 front-proxy 的跟踪信息。
单击一个 trace 来显示从 front-proxy 到 service1 再到 service2 的路径信息，以及每个跳跃点所产生的延迟。
